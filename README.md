...
- Visible Agent orchestration: `agent_orchestration` lists the live Agent manager, serial-by-default policy, managed multi-agent plan cards with milestones, remote-runner contracts/artifact trails, spawn/batch-spawn decision cards, and safe control routes; `agent_orchestration_agent` inspects one visible subagent with its plan card. Actual spawn, message, wait, and cancel actions stay on the first-class `agent` tool.
