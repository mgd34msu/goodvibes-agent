...
- Local long-running commands: `background_processes` and `background_process` inspect tracked ProcessManager jobs with bounded redacted output; confirmed `run_background_process` starts, waits on, or stops one visible local process. It accepts process-style `poll`, `log`, `kill`, `write`, and `sessionId` aliases while `processAction:"capabilities"` reports the core process parity matrix for terminal background start, process list/poll/wait/log/kill/write, PTY, and sudo without overclaiming unsupported interactive routes.
