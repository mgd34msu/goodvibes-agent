...
- Product posture: `channels`, `notifications`, `provider_accounts`, `mcp_servers`, `setup action:"status"` or detailed `setup_posture`, `project_context`, `agent_orchestration`, `model_routing`, `execution_posture`, `background_processes`, `personal_ops action:"briefing|status|intake|lane|read"`, `memory_posture`, `document_ops`, `pairing_posture`, `delegation_posture`, `security_posture`, `support_bundles`, `media_posture`, `sessions`.
...
- Connected host: `service_posture`, `service_endpoint`, `connected_host`, `connected_host_status`, `connected_host_capability`; `daemon` and `daemon_status` are aliases for connected-host posture/status. The connected browser cockpit/PWA is surfaced as `ui_surface` id `connected-browser-cockpit` and opens only through confirmed `open_ui_surface`.
