...
function formatBriefing(baseUrl: string, results: readonly OperatorRouteResult[]): string {
  ...
  return [
    ...
    '  policy: read-only public operator routes; no daemon lifecycle, mutation routes, local workers, WRFC, default Knowledge/Wiki, or HomeGraph',
    '',
  ...
  return {
    definition: {
      ...
      description: [
        ...
        'It never uses default Knowledge/Wiki, HomeGraph, channel send routes, mutation routes, daemon lifecycle, local workers, or WRFC.',
      ].join(' '),
      parameters: {
        type: 'object',
        properties: {},
