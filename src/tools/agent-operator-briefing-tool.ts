...
function formatBriefing(baseUrl: string, results: readonly OperatorRouteResult[]): string {
  ...
  return [
    ...
    '  policy: read-only public operator routes; no runtime-host lifecycle, mutation routes, local workers, WRFC, default Knowledge/Wiki, or non-Agent knowledge segments',
    '',
  ...
  return {
    definition: {
      ...
      description: [
        ...
        'It never uses default Knowledge/Wiki, non-Agent knowledge segments, channel send routes, mutation routes, runtime-host lifecycle, local workers, or WRFC.',
      ].join(' '),
      parameters: {
        type: 'object',
        properties: {},
