...
function buildAgentCapabilitiesContract(registry?: ToolRegistry): Record<string, unknown> {
  ...
  return {
    ...
    canDoNow: [
      ...
      can: 'Create, revise, version, review, comment on, suggest changes to, attach or insert saved artifacts into, and export Agent document drafts with reviewer appendices; browse saved artifacts; track visible research run checkpoints; capture and review research source queues; save sourced research report artifacts; promote reviewed artifacts into isolated Agent Knowledge; handle uploads, source checks, generated media artifacts, and run confirmed blind model comparisons from prompts or saved text artifacts with durable review, side-by-side views, judgment, analytics/synthesis, reviewer handoff, and route-update actions.',
