  ...
  return {
    ...
    canDoNow: [
      ...
      },
      {
        area: 'Web research',
        can: has('web_search')
          ? 'Search the web with bounded read-only evidence, inspect URLs, capture candidate sources in a local review queue, and summarize sourced findings in the main conversation.'
          : 'Web research is not available until the web_search tool is registered.',
        tools: [optionalTool('web_search'), optionalTool('fetch'), optionalTool('agent_research_runs'), optionalTool('agent_research_sources'), optionalTool('agent_research_report')],
        inspect: 'agent_harness mode:"research_runs", mode:"research_queue", or mode:"tool" toolName:"web_search"',
      },
      {
      ...
      can: 'Create, revise, version, review, comment on, suggest changes to, attach or insert saved artifacts into, and export Agent document drafts; browse saved artifacts; track visible research run checkpoints; capture and review research source queues; save sourced research report artifacts; promote reviewed artifacts into isolated Agent Knowledge; handle uploads, source checks, generated media artifacts, and run confirmed blind model comparisons from prompts or saved text artifacts with durable review, judgment, and route-update actions.',
      ...
      },
      {
        ...
        can: 'Inspect configured channel readiness, visible autonomy queue, and send one confirmed message/notification/reminder/media request through configured targets.',
      ...
      },
      {
        ...
        can: 'Inspect provider accounts, subscriptions, model routing, local model cookbook, setup posture, service posture, and connected-host status; apply supported settings changes when explicitly requested.',
    ...
    needsSetupOrIntegration: [
      ...
      'Long-running autonomous operation requires an explicit schedule, routine, automation, or connected-host route visible in the autonomy queue. The main conversation does not silently create hidden background jobs.',
    ...
    commonRoutes: {
      ...
      autonomyQueue: 'agent_harness mode:"autonomy_queue"; inspect one card with mode:"autonomy_queue_item"',
      ...
      researchRuns: 'agent_harness mode:"research_runs"; create/checkpoint/pause/resume/cancel/complete visible run state with agent_research_runs; no hidden background work starts from the ledger',
      researchSources: 'agent_harness mode:"research_queue"; capture/review with agent_research_sources; no Knowledge ingest occurs unless agent_knowledge_ingest is called separately',
      researchReports: 'agent_research_report with reviewed sources, citation coverage metadata, optional requireCitationCoverage:true, confirm:true, and explicitUserRequest',
      localModelCookbook: 'agent_harness mode:"model_routing" query:"local"; inspect with mode:"model_route" modelRouteId:"local-model-cookbook"',
