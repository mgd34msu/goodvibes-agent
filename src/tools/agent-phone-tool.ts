/**
 * agent-phone-tool.ts — the native `phone` tool.
 *
 * Uses a paired phone's camera, screen, location, clipboard, and device
 * commands as agent capabilities. This is a first-class tool on the agent's own
 * contract — not an MCP server — per the standing design constraint.
 *
 * Every capture or effect goes through the SDK's device capability service, so
 * the confirmation gate, the durable-grant lookup, the 24h capture retention,
 * and the housekeeping disclosure are the same no matter which surface or
 * caller triggered it. The tool itself holds no authority and caches no
 * decision: it shapes arguments, calls the service, and renders the outcome.
 */
...
function resolveNodeId(service: PhoneDeviceService, args: PhoneToolArgs, capabilityId: DeviceCapabilityId): string | PhonePayload {
...
async function handleAction(service: PhoneDeviceService, args: PhoneToolArgs): Promise<PhonePayload> {
  {
...
export function registerAgentPhoneTool(registry: ToolRegistry, service: PhoneDeviceService): void {
  if (!registry.has('phone')) registry.register(createAgentPhoneTool(service));
}
