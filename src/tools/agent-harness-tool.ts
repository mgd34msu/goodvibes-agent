...
function detailedHarnessModelAccessGuide(): Record<string, string> {
  return {
    ...
    backgroundProcesses: 'Use terminal background:true to start visible tracked local commands and process action:list|poll|log|wait|kill|write to manage them; lower-level mode:"background_processes", mode:"background_process", and mode:"run_background_process" remain available for route inspection. processAction:"capabilities" probes SDK/daemon interactive contracts; write dispatches only when a safe ProcessManager stdin method exists and is explicitly confirmed; PTY/sudo stay typed-contract or foreground-only boundaries.',
    ...
    operatorMethods: 'List mode:"operator_methods"; inspect mode:"operator_method"; use schedule action:"list|create|remind|edit|run|pause|resume|delete" for schedule UX; run exact daemon methods with agent_operator_method; write/admin routes require confirmation.',
