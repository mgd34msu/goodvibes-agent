...
function stubFetch(latestTag: string): UpdateFetchLike {
  return async (url: string) => {
    ...
    return {
      ...
      status: 302,
      url,
      headers: { get: (name: string) => (name.toLowerCase() === 'location' ? `https://github.com/mgd34msu/goodvibes-agent/releases/tag/${latestTag}` : null) },
      text: async () => '',
...
describe('runLaunchAutoUpdate', () => {
  ...
  test('package-manager and from-source installs never self-update at launch — each skip states itself honestly', async () => {
    const cases = [
      ...
      },
      {
        ...
        line: 'auto-update skipped: running from source (dev checkout)',
      },
    ];
    for (const { execPath, line } of cases) {
  ...
  test('a failed install states the failure and starts the current version', async () => {
    const { options, printed } = baseOptions({
      apply: async () => {
        throw new Error('checksum mismatch for goodvibes-agent-linux-x64');
      },
    });
    const outcome = await runLaunchAutoUpdate(options);
...
describe('restartOntoUpdatedBinary', () => {
  test('spawns the swapped binary with the original argv, marks the child env, and returns its exit code', () => {
    ...
    expect(spawned[0]!.execPath).toBe('/opt/goodvibes/goodvibes-agent');
