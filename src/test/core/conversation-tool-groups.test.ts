...
describe('computeToolGroupMembership', () => {
  ...
  test('records each member tool name in order for the group header summary', () => {
    const messages: Message[] = [
  ...
  test('a result with no recorded tool name falls back to "tool"', () => {
    const messages: Message[] = [
      assistantWithTools(['a', 'b']),
      { role: 'tool', callId: 'a', content: 'x' },
      { role: 'tool', callId: 'b', content: 'y' },
    ];
    expect(computeToolGroupMembership(messages, 0, WIDTH).get(1)?.toolNames).toEqual(['tool', 'tool']);
  });
