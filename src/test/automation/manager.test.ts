...
describe('AutomationManager', () => {
  ...
  // shape as the route-binding fix.
  //
  ...
  describe('automation.enabled feature gate', () => {
    function managerWithGate(enabled: boolean): AutomationManager {
      const configManager = new ConfigManager({ surfaceRoot: 'agent', workingDir: root, configDir: join(root, '.goodvibes', 'agent') });
    ...
    test('automation.enabled true allows a create, and is the shipped default', async () => {
      ...
      const unsetConfig = new ConfigManager({ surfaceRoot: 'agent', workingDir: unsetRoot, configDir: join(unsetRoot, '.goodvibes', 'unset') });
