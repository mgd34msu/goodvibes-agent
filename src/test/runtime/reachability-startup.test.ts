...
describe('the startup check fires on an earlier-PATH copy', () => {
  test('it names both copies, both versions, and the exact fix', async () => {
    const host: FakeHost = {
      probes: [],
      files: {
        [`${BUN_BIN}/goodvibes-agent`]: { link: BUN_PACKAGE, version: 'goodvibes-agent 1.18.1' },
        [`${INSTALL_DIR}/goodvibes-agent`]: { version: 'goodvibes-agent 1.25.0' },
      },
    };
    const result = await runReachabilityCheck({
      execPath: `${INSTALL_DIR}/goodvibes-agent`,
    ...
    expect(text).toContain(`${BUN_BIN}/goodvibes-agent`);
    expect(text).toContain('version 1.18.1');
    expect(text).toContain(`${INSTALL_DIR}/goodvibes-agent`);
...
describe('the startup check stays silent when there is nothing to say', () => {
  test('one copy, first on PATH, current version', async () => {
    const host: FakeHost = {
      probes: [],
      files: { [`${INSTALL_DIR}/goodvibes-agent`]: { version: 'goodvibes-agent 1.25.0' } },
    };
    const result = await runReachabilityCheck({
      execPath: `${INSTALL_DIR}/goodvibes-agent`,
  ...
  test('a source checkout is not an install and is never flagged', async () => {
    const host: FakeHost = {
      probes: [],
      files: { [`${BUN_BIN}/goodvibes-agent`]: { link: BUN_PACKAGE, version: 'goodvibes-agent 1.18.1' } },
    };
    const result = await runReachabilityCheck({
  ...
  test('a package-managed install running from inside node_modules says nothing about PATH', async () => {
    const host: FakeHost = {
      probes: [],
      files: { [BUN_PACKAGE]: { version: 'goodvibes-agent 1.18.1' } },
    };
    const result = await runReachabilityCheck({
...
describe('being behind is stated plainly, and only when it is known', () => {
  test('a package-managed install that cannot swap itself is told it is behind, with its own command', async () => {
    const host: FakeHost = {
      probes: [],
      files: {
        [BUN_PACKAGE]: { version: 'goodvibes-agent 1.18.1' },
        [`${BUN_BIN}/goodvibes-agent`]: { link: BUN_PACKAGE, version: 'goodvibes-agent 1.18.1' },
      },
    };
    const result = await runReachabilityCheck({
    ...
    expect(text).toContain('bun add -g @pellux/goodvibes-agent');
  });
  ...
  test('an unknown latest version says nothing rather than guessing', () => {
    const scan = scanCommandShadows({
      ...
      homeDir: HOME,
      isExecutableFile: (path) => path === `${INSTALL_DIR}/goodvibes-agent`,
      realPath: (path) => path,
    });
...
describe('resolveSelfDirectory', () => {
  test('prefers the PATH entry that actually resolves to this executable', () => {
    expect(resolveSelfDirectory({
      ...
      realPath: (path) => (path === `${BUN_BIN}/goodvibes-agent` ? BUN_PACKAGE : path),
      isExecutableFile: (path) => path === `${BUN_BIN}/goodvibes-agent`,
    })).toBe(BUN_BIN);
  });
