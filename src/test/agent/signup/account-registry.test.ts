...
import {
  ...
  accountRegistryStorePath,
  MAX_ACCOUNT_RECORDS,
} from '../../../agent/signup/account-registry.ts';
...
function tempRegistry(): Harness {
  const root = mkdtempSync(join(tmpdir(), 'goodvibes-agent-signup-'));
  const shellPaths = createShellPathService({ workingDirectory: root, homeDirectory: root });
  return {
    registry: AgentAccountRegistry.fromShellPaths(shellPaths),
    storePath: accountRegistryStorePath(shellPaths),
  };
}
