...
describe('composeDraftReply', () => {
  ...
  it('returns a draft with requiresBeforeSendReview = true', () => {
    const result = composeDraftReply(inbound, defaultProfile);
  ...
  it('throws when context contains secret-like text', () => {
    expect(() =>
      composeDraftReply(inbound, defaultProfile, 'token: abc123secretpassword'),
    ).toThrow(/secret-like text/);
  });

  it('is deterministic — same inputs produce identical output', () => {
    const a = composeDraftReply(inbound, defaultProfile, 'some context');
    const b = composeDraftReply(inbound, defaultProfile, 'some context');
  ...
  it('handles inbound with no sender name gracefully', () => {
    ...
    const result = composeDraftReply(noName, defaultProfile);
    expect(result.body).toBeTruthy();
    expect(result.requiresBeforeSendReview).toBe(true);
  });
});
