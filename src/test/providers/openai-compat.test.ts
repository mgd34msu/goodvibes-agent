...
// The provider calls `this.client.chat.completions.create(...).withResponse()` (added
// so success-path rate-limit headers are readable, not only on 429s) — the real openai
// SDK's `create()` returns an APIPromise with a `.withResponse()` method that resolves to
// `{ data, response }`. A bare async function standing in for `create` returns a plain
// Promise with no `.withResponse()`, so calling it throws a TypeError before the intended
// stub error/stream is ever reached. Wrap the stub so `.withResponse()` proxies to it.
