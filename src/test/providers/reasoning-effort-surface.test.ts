...
// four levels (instant, low, medium, high). That offered 'instant' to models
// that reject it, hid 'none', 'minimal', 'xhigh' and 'max' from models that
// accept them, and printed a fixed provider wire explainer regardless of which
...
describe('effort presentation per spec kind', () => {
  test('a named-levels model offers exactly its own levels, not a fixed four', () => {
    ...
    // The levels a hardcoded list used to invent for every model.
    ...
    // the old behaviour for anything outside a fixed four-level table.
...
describe('the /effort explainer is generated from the resolved spec', () => {
  test('it names the field this model actually receives, not a fixed provider list', () => {
    ...
    // The old /effort command printed this exact four-provider block for
    // every model, regardless of which one was actually serving.
    expect(text).not.toContain('Mercury-2');
    expect(text).not.toContain('GPT-5 (no-op)');
...
describe('switching or failing over re-resolves the level against the serving model', () => {
  ...
  test('a level the serving model does offer is left alone and says nothing', () => {
    const remapped = remapEffortForServingModel('xhigh', model(EFFORT_SPEC));
