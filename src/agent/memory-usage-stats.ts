...
import type { MemoryConsolidationUsageSignal } from './memory-consolidation.ts';

/**
 * Per-memory usage counters answering "was injected context actually used?".
 *
 ...
 * only. It is instrumentation data, not a second memory store — same durable
 * JSON sidecar idiom as the prompt-context and consolidation receipts.
 *
 * The counters are queryable through the memory posture surface and feed the idle
 * consolidation decay ordering (never-referenced memories decay first) via
 * `lookup`.
 */
