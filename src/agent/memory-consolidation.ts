...
import type { ResolvedMemoryConsolidationConfig } from './memory-consolidation-config.ts';

/**
 * Idle-time memory consolidation — the housekeeping pass that runs over already
 * stored memory records when the agent is genuinely idle.
 *
 * It performs only REVERSIBLE operations on existing records: it merges duplicate
 * records into a survivor and marks the losers stale (never deletes), and it
 * decays never-referenced, aged records (lowering confidence, then marking stale
 * once the confidence floor is crossed). Anything that would require a NEW
 ...
 *
 * HOIST CANDIDATE: the decide-what-to-do logic here is pure over an injected
 * record set + usage lookup + clock. If a second surface (webui, daemon) needs
 * the same behavior it should move to the SDK as a policy with injectable I/O;
 * the MemoryRegistry writes below are the only surface-coupled part.
 */
...
export function runMemoryConsolidation(input: MemoryConsolidationInput): MemoryConsolidationRunReceipt {
  ...
  const runId = `mcon-${input.now.toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  return {
    runId,
