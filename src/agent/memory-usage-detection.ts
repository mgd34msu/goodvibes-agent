/**
 * Heuristic reference detection: did the model's output plausibly USE an injected
 * memory, or was the memory merely present in the prompt with no detectable trace?
 *
 * This is deliberately an honest, coarse signal with exactly two tiers:
 ...
 * overlap (see MEMORY_USAGE_SIGNAL_NOTE). The bar is set to require either two
 * distinctive tokens, one long distinctive token, or a distinctive adjacent
 * phrase, so common words alone never count as a reference.
 */
