...
export const COMPETITIVE_FEATURE_INVENTORY: readonly CompetitiveFeatureInventoryItem[] = [
  ...
  },
  {
    ...
    goodVibesStatus: 'partial',
    owners: ['agent'],
    goodVibesNow: 'Agent has local memory, notes, personas, skills, routines, learned-behavior capture, safe VIBE.md personality discovery from project/global files, prompt injection for VIBE.md after secret-looking content scans, and /vibe status/init/show/import-persona routes so users can start with a friendly personality file instead of persona-registry ceremony. Formal behavior prompt injection still uses only reviewed memory at or above the durable confidence threshold plus reviewed setup-ready skills, routines, bundles, and personas while listing suppressed unreviewed/setup-blocked behavior for curator review. The read-only learning curator ranks review, stale, missing-setup, low-confidence, duplicate-consolidation candidates with visible diffs/rollback routes, an ordered duplicate-consolidation batch review plan, confirmed duplicate-consolidation phase helpers for preview/merge/stale/delete/rollback with durable receipts and delete refusal until duplicates are staged stale, reviewed-note, completed-work, completed-research, and saved-session memory/behavior proposals, and promotion candidates with existing safe routes.',
    nextMoves: [
      'Surface blocked or truncated VIBE.md files in the learning curator and setup checklist so personality issues are visible before they surprise a user.',
      'Let profile starter export/import carry VIBE.md alongside personas, skills, routines, and setup when the user chooses that profile scope.',
      'Expose external memory-provider setup/status when the connected host or SDK publishes Honcho, Mem0, Supermemory, or similar backend records.',
