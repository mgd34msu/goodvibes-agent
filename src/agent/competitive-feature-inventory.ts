...
export const COMPETITIVE_FEATURE_INVENTORY: readonly CompetitiveFeatureInventoryItem[] = [
  ...
  },
  {
    ...
    goodVibesNow: 'Agent has onboarding, diagnostics, redacted GoodVibes settings import preview/apply, a visible Start checklist that keeps connected-host auth and install smoke beside runtime/model setup, and a model-visible first-run setup plan that orders connected-host readiness, connected-host auth, provider/model access, install smoke, local model readiness, Agent Knowledge, local behavior, channels, automation review, browser/desktop control, delegation, and finish state with exact user and model routes plus primary handoff cards for every blocked, check, or recommended setup row. Connected-host setup includes live service probe evidence, token-safe auth posture with exact pairing route ids, confirmed SDK-backed local operator-token create/repair with no raw-token output, recommended diagnostic/status cards, confirmed service install/start/restart routes that stay inspect-first unless service status proves need, offline bootstrap commands for missing-host setup, a confirmed token-safe setup smoke route with optional durable redacted evidence artifacts from package binary to first assistant turn, Home/setup summary surfacing for the latest smoke result plus smoke history/trend/frequent blockers, and fixtures for missing host, unreachable host, reachable host, missing token, and model-unconfigured paths.',
    nextMoves: [
      'Collapse the visible checklist and harness handoff cards into a single guided wizard with progress, backtracking, and setup-smoke rerun.',
      'Use saved setup smoke history to focus the checklist on the blocker that has repeated most often.',
    ],
    competitorSignals: [
  ...
  },
  {
    ...
    nextMoves: [
      ...
      'Extend Personal Ops live records to first-class inbox and agenda records once connectors expose them, while deepening provider-backed task/reminder records beyond the current local/connected-host operation cards.',
    ],
    competitorSignals: [
  ...
  },
  {
    ...
    goodVibesStatus: 'partial',
    owners: ['agent'],
    goodVibesNow: 'Agent has local memory, notes, personas, skills, routines, learned-behavior capture, safe VIBE.md personality discovery from project/global files, safe project context discovery for .hermes.md/HERMES.md/AGENTS.md/CLAUDE.md/HERMES_HOME/SOUL.md/.cursorrules/.cursor/rules/*.mdc files, prompt injection for VIBE.md and project context after secret-looking content scans, /vibe status/init/show/import-persona routes, setup posture, learning-curator personality health cards for blocked/truncated personality files, and opt-in profile starter export/import/application for VIBE.md so users can start with a friendly personality file without persona-registry ceremony, hidden prompt surprises, or profile portability gaps. `agent_harness` now exposes `project_context` and `project_context_file` for target-aware context inspection, loaded/truncated/blocked status, and bounded bodies. Formal behavior prompt injection still uses only reviewed memory at or above the durable confidence threshold plus reviewed setup-ready skills, routines, bundles, and personas while listing suppressed unreviewed/setup-blocked behavior for curator review. The read-only learning curator ranks review, stale, missing-setup, low-confidence, VIBE.md health, duplicate-consolidation candidates with visible diffs/rollback routes, an ordered duplicate-consolidation batch review plan, confirmed duplicate-consolidation phase helpers for preview/merge/stale/delete/rollback with durable receipts and delete refusal until duplicates are staged stale, reviewed-note, completed-work, completed-research, and saved-session memory/behavior proposals, and promotion candidates with existing safe routes. The `memory_posture` harness mode now exposes local memory counts, prompt-active recall, vector stats, embedding-provider doctor warnings, provider inspection, and an honest not-published status for Honcho/Mem0/Supermemory-style external memory backends until the SDK/daemon publishes concrete records.',
    nextMoves: [
      'Wire external memory-provider setup/status to the memory_posture provider records when the connected host or SDK publishes Honcho, Mem0, Supermemory, or similar backend records.',
  ...
  },
  {
    ...
    goodVibesStatus: 'partial',
    owners: ['agent', 'connected-host'],
    goodVibesNow: 'Agent has confirmed natural-language autonomous schedule creation when task, cadence, success criteria, and user request provenance are explicit; reminder scheduling; routine promotion; confirmed connected schedule editing with read-only current-state before/after diffs from schedules.list before confirmation; operator action tools; connected schedule posture; a read-only ongoing-work intake selector with visible webhook/event-trigger setup detection and operator-method discovery; and a read-only autonomy queue that maps visible owners, status, inspect routes, cancel/recovery routes, live research runs, live connected-host task records, live approval records, live automation run records, live schedule records, delegated subagent orchestration routes, log tails, diagnostics for task retry/output/correlation and automation telemetry/delivery/route posture, source ids, normalized available/unavailable controls with reasons, and exact confirmed checkpoint/pause/resume/cancel/edit/control routes where supported, including first-class schedule pause/resume aliases over the daemon enable/disable lifecycle. Agent also exposes visible local Agent orchestration through `agent_orchestration` and `agent_orchestration_agent`, plus tracked local background processes through `background_processes`, `background_process`, and confirmed `run_background_process` start/wait/stop routes that feed the same background-work cockpit lane; richer connected-host output streams still need depth.',
    nextMoves: [
      'Attach deeper host output streams beyond current task and automation diagnostics when the connected host exposes them.',
      'Add confirmed webhook and event-trigger creation routes when the connected host exposes scoped trigger records.',
    ],
    competitorSignals: [
  ...
  },
  {
    ...
    goodVibesStatus: 'partial',
    owners: ['agent', 'connected-host', 'companion'],
    goodVibesNow: 'Agent exposes local-first execution posture for read/search/analyze, file edit/write, bounded foreground shell commands, web/fetch evidence, visible process monitor/live tail/tool inspector supervision routes, confirmed tracked local background process start/list/status/log/wait/stop routes over the shared ProcessManager, a read-only process parity matrix for terminal background start plus process list/poll/wait/log/kill/write/PTY/sudo semantics, bounded redacted execution history records, confirmed file recovery, strict browser/desktop ready-attention-setup posture with workflow cards/checklists/fallback routes, and delegation for isolation, parallelism, remote execution, separate worktrees, or requested review. PTY/stdin write is honestly reported as unsupported until the shared substrate exposes it safely, and background sudo prompts are blocked with foreground/user-supervised guidance and non-secret SUDO_PASSWORD presence posture.',
    nextMoves: [
      ...
      'Add safe PTY and stdin write support once the SDK/daemon ProcessManager exposes an interactive API with visible user control.',
      'Expose richer sudo setup posture and SUDO_PASSWORD guidance without ever storing or printing raw password values.',
  ...
  },
  {
    ...
    goodVibesStatus: 'partial',
    owners: ['agent', 'connected-host'],
    goodVibesNow: 'GoodVibes has shared-session, remote runner, artifact, task, worktree, orchestration, subagent, and delegation foundations; Agent exposes `agent_orchestration` and `agent_orchestration_agent` for live visible subagent records, serial-by-default policy, spawn/batch-spawn decision cards, templates, and exact first-class `agent` list/inspect/message/wait/cancel routes, plus local-first, TUI handoff, delegated-review, remote-inspection, and hidden-fanout-blocked decision cards with structured confirmed handoff briefs. It intentionally blocks invisible local fanout and raw remote mutation from Agent, and does not yet turn one approved work plan into a fully managed cancellable multi-runner plan with user-facing milestones.',
    nextMoves: [
