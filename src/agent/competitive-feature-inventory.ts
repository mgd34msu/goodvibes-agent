...
export const COMPETITIVE_FEATURE_INVENTORY: readonly CompetitiveFeatureInventoryItem[] = [
  ...
  },
  {
    ...
    nextMoves: [
      ...
      'Add safe PTY, stdin write, and sudo credential mediation once the SDK/daemon ProcessManager exposes an interactive API with visible user control.',
  ...
  },
  {
    ...
    goodVibesStatus: 'partial',
    owners: ['agent', 'connected-host'],
    goodVibesNow: 'GoodVibes has shared-session, remote runner, artifact, task, worktree, orchestration, subagent, and delegation foundations; Agent exposes `agent_orchestration` and `agent_orchestration_agent` for live visible subagent records, serial-by-default policy, managed multi-agent plan milestones, per-agent plan cards with cancel/message/wait routes, remote-runner contract/artifact evidence, spawn/batch-spawn decision cards, templates, and exact first-class `agent` list/inspect/message/wait/cancel routes, plus local-first, TUI handoff, delegated-review, remote-inspection, and hidden-fanout-blocked decision cards with structured confirmed handoff briefs. Approved visible work-plan items can now be dispatched through `agent_work_plan action:"dispatch_agents"` into first-class `agent` spawn or batch-spawn calls with saved linked-agent receipts. It intentionally blocks invisible local fanout and raw remote mutation from Agent.',
    nextMoves: [
      'Attach remote artifacts automatically when visible runners complete and the remote runtime exposes safe capture/export hooks.',
      'Surface work-plan dispatch receipts directly in the managed orchestration cards so closeout is one step from the plan.',
