...
export const COMPETITIVE_FEATURE_INVENTORY: readonly CompetitiveFeatureInventoryItem[] = [
  ...
  },
  {
    ...
    goodVibesNow: 'Provider routing, subscription posture, local compatible provider discovery, model pickers, visible route-readiness inspection, and a hardware-scored local model cookbook for Ollama, llama.cpp, vLLM, and local OpenAI-compatible servers exist; model routes and local recipes expose one readiness score across latency, context window, tool support, vision, cost, and privacy. The cookbook scans local CPU/RAM/platform, applies safe accelerator hints, ranks fit, returns setup plans with download/start guidance, local endpoint candidates with exact endpoint inspect routes, model-list smoke commands, smoke success criteria, failure triage, confirmed run_local_model_smoke model-list checks, provider-add and refresh route hints, benchmark action routes, a visible Check local servers action, a visible model-lane local benchmark action backed by agent_model_compare, saved benchmark-evidence review, saved local-route benchmark artifacts, and revealed winner judgments that raise matching recipe confidence before any separate default-model apply action.',
    nextMoves: [
  ...
  },
  {
    ...
    goodVibesStatus: 'partial',
    owners: ['agent', 'connected-host', 'companion'],
    goodVibesNow: 'Agent now has a first-class `device action:"status|capability|browser|control|voice|provider|open_browser|open_tts_provider|open_tts_voice"` UX over companion pairing, mobile/PWA, voice/TTS, notifications, browser/desktop control, provider posture, and visible browser/TTS picker handoffs while preserving the existing harness detail routes. The read-only device capability map reports ready/attention/setup-needed states, browser/PWA readiness reports URL/category/mobile/receipt gaps, browser/desktop control reports trusted-tool/MCP setup posture, and voice workflow posture maps push-to-talk, voice memo transcription, spoken responses, and wake-word capture. The Voice & Media workspace now gives users Voice workflows, Device capability map, and Browser/PWA readiness actions with direct `device` route hints; camera, location, and wake-word capture remain honest not-published contract gaps.',
    nextMoves: [
