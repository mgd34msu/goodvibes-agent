 ...
 *
 * Follows the conventions in `src/agent/calendar-registry.ts` — store path from an
 * injected path service, versioned store file, defensive parse, atomic write — with one
 * deliberate divergence noted at `readStore`.
 */
 ...
 */
function parseAccount(value: unknown): AgentAccountRecord | null {
  ...
  if ([serviceUrl, purpose, credentialSecretKey, aliasAddress].some(containsSecretLikeText)) return null;
...
function parseStore(raw: string): ParsedStore {
  ...
  const accounts = entries.map(parseAccount).filter((entry): entry is AgentAccountRecord => entry !== null);
...
export function accountRegistryStorePath(shellPaths: AgentLocalStorePaths): string {
  return shellPaths.resolveUserPath(GOODVIBES_AGENT_SURFACE_ROOT, 'signup', 'accounts.json');
}
...
export class AgentAccountRegistry {
  ...
  public static fromShellPaths(shellPaths: AgentLocalStorePaths): AgentAccountRegistry {
    return new AgentAccountRegistry(accountRegistryStorePath(shellPaths));
  }
  ...
  public record(input: AgentAccountCreateInput): AgentAccountRecord {
    ...
    if ([serviceUrl, purpose, credentialSecretKey, aliasAddress].some(containsSecretLikeText)) {
