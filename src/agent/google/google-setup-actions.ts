...
import {
  ...
  OAUTH_SCOPES,
  REQUIRED_SERVICES,
} from './google-setup-plan.ts';
import type { GoogleStepRunner, GoogleStepRunnerResult } from './google-setup-flow.ts';
import type {
  GoogleBrowserPort,
  GoogleCommandPort,
  GoogleConfigPort,
  ...
  GoogleSetupPath,
  GoogleStepId,
} from './google-setup-types.ts';
import { createAppPassword } from './google-app-password-flow.ts';
import { createDesktopOAuthClient, publishApp, readPublishingStatus } from './google-console-flow.ts';
import {
  clientCredentialsFromInput,
  readClientCredentialsFromJson,
  type GoogleClientCredentials,
  type GoogleClientIntakeResult,
} from './google-client-intake.ts';
import { collectDownloadedClientFile, type DownloadScanPort } from './google-client-download.ts';
import {
  checkAuthenticated,
  detectGcloud,
  enableServices,
...
} from './google-oauth-loopback.ts';
import { adoptGmailMcpCredentials, type GoogleFilePort } from './google-credential-adoption.ts';
...
function signedInRunner(deps: GoogleSetupActionDeps): GoogleStepRunner {
  return async (spec) => {
    ...
    const { looksLikeGoogleSignIn } = await import('./google-browser-port.ts');
...
function oauthAuthorizeRunner(deps: GoogleSetupActionDeps): GoogleStepRunner {
  return async () => {
    ...
    const listener = startLoopbackListener({ expectedState: state });
    try {
