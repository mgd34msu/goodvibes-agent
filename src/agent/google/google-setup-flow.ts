...
import { runbookAnchor, stepsForPath } from './google-setup-plan.ts';
import type {
  GoogleProgressPort,
  GoogleSetupPath,
  GoogleSetupReport,
...
function failureFromThrow(spec: GoogleSetupStepSpec, error: unknown): GoogleStepRunnerResult {
  ...
  return {
    ...
    fix: `Follow the written steps for "${spec.title}" in docs/google-setup-runbook.md${runbookAnchor(spec.id)}, then re-run — completed work is detected and skipped.`,
  };
}
...
export function renderGoogleSetupReport(report: GoogleSetupReport): string {
  ...
  if (!report.ok) {
    lines.push('');
    lines.push('Written instructions for every step: docs/google-setup-runbook.md');
  }
