/**
 * google-gcloud.ts — the gcloud driver for the OAuth (Path B) setup flow.
 *
 * Every subprocess call goes through the injected `GoogleCommandPort` (see
 * `google-setup-types.ts`) so this module is fully testable without a real
 * gcloud install, a real Google account, or a real Cloud project. The one
 * real implementation, `createProcessCommandPort()`, is the only place that
 * touches `Bun.spawn`.
 *
...
import type { GoogleCommandPort, GoogleCommandResult } from './google-setup-types.ts';
