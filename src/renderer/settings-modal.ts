...
const CATEGORY_INFO: Record<SettingsCategory, string> = {
  ...
  voice: 'Two independent voice capabilities. Free local voice engines (voice.local.*) as the peer beside the premium provider route above: configurable-not-configured by default, every key ships empty, nothing auto-downloads, and an unset engine reports an honest unconfigured status rather than an error — each engine needs one explicit setup action. Wake-word detection (voice.wake.*, rendered as its own unit below): listening continuously for a spoken wake phrase and handing the utterance that follows to speech-to-text. Off by default, and delivery to THIS surface is off by default too (voice.wake.surfaces.agent), because two terminal surfaces both acting on one spoken utterance is a confusing default — but it is LIVE here when you turn both on: a recorder subprocess (voice.wake.captureCommand) feeds the pinned classifier through a WASM runtime, voice.wake.activationSound plays at a confirmed wake, a persistent listening row shows in the footer per voice.wake.indicator, and what you say next goes to speech-to-text and lands in the conversation input — or is sent straight away with voice.wake.autoSubmit. Two rows are not in force: voice.wake.vadThreshold above 0 refuses to start the detector because no voice-activity model is pinned, and voice.wake.noiseSuppression "speex" refuses for the same kind of reason (no surface applies that stage). Nothing downloads on its own — the models come from /voice wake setup, and until then an enabled detector says so rather than pretending to listen. Its published recall figures are measured on synthesised speech only, so no human recording of the phrase is behind them.',
