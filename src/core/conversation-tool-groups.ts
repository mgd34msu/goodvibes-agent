 ...
 * effects. Called once per render pass (appendConversationMessages) over the
 * currently-visible message slice, with the absolute index of the slice's
 * first message so group keys and member indexes line up with the same
 * collapseKey / messageLineRegistry indices the rest of the render uses
 * (messages may be a post-clearDisplay slice, not the full snapshot — see
 * appendConversationMessages's msgIndexOffset doc).
 *
 * Ported from goodvibes-tui's module of the same name.
 */

import type { ConversationMessageSnapshot } from '@pellux/goodvibes-sdk/platform/core';
import { countExpandedToolResultLines } from '../renderer/tool-result-expanded-lines.ts';
...
/** Build the per-tool-message membership lookup used by the renderer.
  ...
  for (const group of groups) {
    ...
    for (const absIdx of group.toolMessageIndexes) {
      ...
      if (message && message.role === 'tool') {
        totalLines += countExpandedToolResultLines(message.content, width);
