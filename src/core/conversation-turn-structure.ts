 ...
 * no schema change: a subagent keeps its own ConversationManager, and the
 * caller supplies a resolver (RenderPlanOptions.resolveAgentSnapshot) that
 * reads that manager's message snapshot live. So when a tool call spawned an
 * agent, the plan SPLICES that agent's own plan in beneath the call,
 * recursively, the identical operation to lifting a result into its call's
 * subtree, applied to a nested snapshot. The parent transcript, the persisted
 * format, and the message schema are untouched; only the render plan nests.
 *
