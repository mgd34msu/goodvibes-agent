/**
 * reasoning-effort-surface.ts, this repo's single reading of the SDK's
 * per-model reasoning-effort spec.
 *
 * Every effort-shaped surface (the `/effort` command, the model picker's
 * effort step, the model-switch path) asks this module what a specific model
 * offers, instead of each one carrying its own list. The list used to be the
 * hardcoded four levels `instant, low, medium, high`, which was wrong in both
 * directions: it offered `instant` to models that reject it and hid
 * `xhigh`/`max`/`none` from models that accept them.
 *
 ...
 */
export function publishActiveEffortOptions(model: EffortModelLike | null | undefined, sessionId?: string): void {
  if (!model) {
    setActiveReasoningEffortOptions(null, sessionId);
    return;
  }
  setActiveReasoningEffortOptions(effortLevelsForModel(model), sessionId);
}
...
function describeEffortLevel(level: string, spec: ReasoningEffortSpec): string {
  const base = EFFORT_DESCRIPTIONS[level as keyof typeof EFFORT_DESCRIPTIONS] ?? '';
 ...
 * spec. Replaces a fixed block that would otherwise name the same providers
 * for every model regardless of which one was actually serving.
 */
 ...
 *
 * Printing the raw config value alone is misleading in three separate ways:
 ...
 *
 * Deliberately structural rather than `ModelDefinition`: some callers carry a
 * lighter record (id, provider, displayName, registryKey) and still need the
 * same resolution.
 */
