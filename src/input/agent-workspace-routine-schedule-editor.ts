...
export function createRoutineScheduleEditor(
  ...
  return {
    ...
    message: `Create one external schedule from a reviewed local routine. Selected: ${selected}. Type yes on the final field to confirm.`,
    fields: [
      ...
      { id: 'scheduleName', label: 'Schedule name', value: selectedRoutine?.name ?? '', required: false, multiline: false, hint: 'Optional display name for the external schedule.' },
