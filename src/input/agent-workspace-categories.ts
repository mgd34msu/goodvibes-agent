...
export const AGENT_WORKSPACE_CATEGORIES: readonly AgentWorkspaceCategory[] = [
  ...
  },
  {
    ...
    detail: 'Agent does not create local automation jobs or hidden scheduler spawns. Reviewed local routines can be promoted into connected schedules only through an explicit schedules.create command with --yes, optional delivery targets, and a redacted local receipt.',
    actions: [
      ...
      { id: 'schedule-policy', label: 'Local scheduler blocked', detail: 'Local schedule add/run/remove/enable/disable remain blocked; only explicit connected schedule promotion is allowed here.', kind: 'guidance', safety: 'blocked' },
