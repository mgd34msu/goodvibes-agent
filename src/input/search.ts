...
export class SearchManager {
   ...
   * copied, bookmarked) while they were expanded, which stay exactly as the
   * user left them (see ConversationManager.restoreSearchExpansions).
  ...
  close(conversationManager?: ConversationManager | null): void {
    conversationManager?.restoreSearchExpansions();
    this.active = false;
    this.locked = false;
  }
  ...
  revealCurrentMatch(history: InfiniteBuffer, conversationManager?: ConversationManager | null): void {
    ...
    for (const key of match.collapseKeys) {
      ...
      conversationManager.markSearchExpanded(key);
    }
