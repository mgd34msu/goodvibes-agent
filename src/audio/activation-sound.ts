 ...
 * otherwise the only feedback is a transcript arriving seconds later, and a false
 * accept is indistinguishable from nothing having happened at all. So the sound is
 * played AT the wake, before transcription, per `voice.wake.activationSound`:
 *
...
import type { VoiceAudioChunk, WakeActivationSound } from '@pellux/goodvibes-sdk/platform/voice';
 ...
 *
 * Written as samples rather than shipped as an asset for the same reason the wake
 * front end is computed in code: a binary blob is one more thing that can be
 * missing, and this one is 140 milliseconds of two sine waves.
 */
...
/**
 * Play the resolved activation sound. Never throws and never blocks the caller on
 * the audio finishing: a wake's next step is recording the utterance, and a sound
 * that fails to play must not delay or cancel that.
 *
 * `sound` arrives already resolved against surface capability — the SDK downgrades
 * `custom` to `chime` on a surface that cannot read a local file, and this surface
 * can, so `custom` here means a real path.
 */
export function playActivationSound(sound: WakeActivationSound, deps: ActivationSoundPlayerDeps): void {
  ...
  if (!deps.player.available) {
    deps.notify(`[Wake] Wake heard, but no audio player is installed to play the ${sound.kind} activation sound (${deps.player.label}).`);
    return;
  }
  let data: Uint8Array;
  let format: string;
  if (sound.kind === 'custom') {
    ...
    if (path.length === 0) {
      deps.notify('[Wake] voice.wake.activationSound is "custom" but voice.wake.activationSoundPath is empty, so no sound was played.');
      return;
    }
    try {
      data = (deps.readFile ?? ((target: string) => new Uint8Array(readFileSync(target))))(path);
    } catch (error) {
      deps.notify(`[Wake] Could not read the activation sound at ${path}: ${error instanceof Error ? error.message : String(error)}`);
      return;
    }
  ...
  void deps.player.play(singleChunk(data, format), { format }).catch((error: unknown) => {
    deps.notify(`[Wake] The activation sound could not be played: ${error instanceof Error ? error.message : String(error)}`);
  });
}
