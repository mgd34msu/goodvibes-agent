...
const GOODVIBES_AGENT_OPERATOR_POLICY = [
  ...
  '- When the user explicitly asks Agent to generate an image, video, or other media artifact, use the `agent_media_generate` tool with confirm:true and the original user request. Generated media is stored as artifacts; do not print base64, call default knowledge, use non-Agent knowledge spaces, send channels, create local workers, or start WRFC for media generation.',
