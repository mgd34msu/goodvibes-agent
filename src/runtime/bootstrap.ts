...
const GOODVIBES_AGENT_OPERATOR_POLICY = [
  ...
  '- Use the `agent_operator_briefing` tool for read-only main-conversation summaries of connected work plan, approvals, automation, schedules, and scheduler capacity. This tool must not call mutation routes, daemon lifecycle routes, default Knowledge/Wiki, HomeGraph, local worker spawning, or WRFC.',
  ...
  '- Use the `agent_knowledge` tool for Agent-owned Knowledge/Wiki status, ask, and search from the main conversation. It must use only /api/goodvibes-agent/knowledge/* and must fail closed instead of falling back to default Knowledge/Wiki, HomeGraph, or non-Agent knowledge spaces.',
  '- When the user explicitly asks Agent to add, import, remember, or ingest a URL, URL-list file, local file, bookmarks file, browser history, or connector input into its wiki/knowledge, use the `agent_knowledge_ingest` tool with confirm:true and the original user request. It must write only to /api/goodvibes-agent/knowledge/* ingest routes and never to default Knowledge/Wiki or HomeGraph.',
  ...
  '- When the user explicitly asks Agent to generate an image, video, or other media artifact, use the `agent_media_generate` tool with confirm:true and the original user request. Generated media is stored as artifacts; do not print base64, call default Knowledge/Wiki, use HomeGraph, send channels, spawn background agents, or start WRFC for media generation.',
