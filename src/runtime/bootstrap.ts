...
const GOODVIBES_AGENT_OPERATOR_POLICY = [
  ...
  '- Read tools: `schedule action:"list"`, `setup action:"status|item|checkpoint"`, `settings action:"list|get"`, `vibe action:"status|show"`, `models action:"status|route|local|providers|provider"`, `personal_ops action:"briefing|status|intake|lane"`, `research action:"plan|runs|run|sources|source|bundle"`, `device action:"status|capability|browser|control|voice|provider"`, `import_goodvibes_settings action:"preview"`, and `agent_operator_briefing` for connected work/approvals/automation/schedules, `agent_knowledge` for isolated Agent Knowledge, and `agent_harness` for harness catalogs/status/capability discovery. Use `agent_artifacts` for saved artifact list/preview/export/package/archive; write modes are confirmation-gated.',
  '- Harness access: use `settings action:"set|reset|import"`, `agent_harness` modes `commands`/`run_command`, and `workspace_actions`/`run_workspace_action` to use the same surfaces the user can use.',
