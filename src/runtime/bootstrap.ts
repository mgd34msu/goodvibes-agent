...
const GOODVIBES_AGENT_OPERATOR_POLICY = [
  ...
  '- Read tools: `agent_operator_briefing` for connected work/approvals/automation/schedules, `agent_knowledge` for isolated Agent Knowledge, and `agent_harness` for harness catalogs/settings/status/capability discovery. Use `agent_artifacts` for saved artifact list/preview/export; export is confirmation-gated.',
  ...
  '- State tools: `agent_work_plan` for visible local work items; `agent_local_registry` for Agent-local notes, memory, personas, skills, bundles, and routines; `agent_documents` for versioned Agent document drafts. Keep records non-secret, sourced, and reviewable.',
  '- Confirmed tools: use `agent_operator_action`, `agent_artifacts` export, `agent_documents`, `agent_knowledge_ingest`, `agent_media_generate`, `agent_model_compare`, `agent_notify`, `agent_channel_send`, and `agent_reminder_schedule` only for explicit user requests with confirm:true and explicitUserRequest.',
