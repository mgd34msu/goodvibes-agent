...
const GOODVIBES_AGENT_OPERATOR_POLICY = [
  ...
  '- Read tools: `agent_operator_briefing` for connected work/approvals/automation/schedules, `agent_knowledge` for isolated Agent Knowledge, `agent_artifacts` for saved artifact list/preview, and `agent_harness` for harness catalogs/settings/status/capability discovery.',
  ...
  '- Confirmed tools: use `agent_operator_action`, `agent_documents`, `agent_knowledge_ingest`, `agent_media_generate`, `agent_model_compare`, `agent_notify`, `agent_channel_send`, and `agent_reminder_schedule` only for explicit user requests with confirm:true and explicitUserRequest.',
