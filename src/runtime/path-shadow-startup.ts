...
export async function runReachabilityCheck(input: {
  ...
  return {
    ...
    notices: buildReachabilityNotices({
      ...
      latestVersion,
      // A binary agent install is placed by the platform installer; a
      // package-managed one is the user's package manager to move.
...
/** The real host inputs, in one place, so both entrypoints below agree. */
async function checkThisProcess(): Promise<ReachabilityCheckResult> {
  return runReachabilityCheck({
...
/**
 * Launch-time wiring, run alongside the launch self-update and BEFORE the
 * runtime bootstrap or any terminal mode change — the same slot the update
 * receipt uses, and for the same reason: if the build about to start is not
 * the one the user reaches, or is not the current release, that has to be the
 * first thing said, not a footnote after a session is already going.
 *
...
export async function reachabilityAtLaunch(params: {
  ...
  const lines: string[] = [];
  try {
    const result = await checkThisProcess();
    ...
    // Best-effort — a reachability check must never block or crash launch.
  }
  return lines;
}

/**
 * Boot-time wiring for surfaces that have a router but no pre-bootstrap slot.
 * Prints one high-priority system message per line so the report reads as
 * prose rather than a single wrapped paragraph, and swallows everything.
 */
