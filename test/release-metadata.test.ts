...
describe('release metadata', () => {
  ...
  test('documents release checklist and manual PTY gate', async () => {
    ...
    expect(changelog).toContain('@pellux/goodvibes-sdk@0.33.34');
    ...
    expect(upgrade).toContain('@pellux/goodvibes-sdk@0.33.34');
    expect(upgrade).toContain('Agent-specific knowledge isolation: active in code');
    expect(upgrade).toContain('## Handoff Summary');
    expect(upgrade).toContain('/api/goodvibes-agent/knowledge/ask');
    expect(evidence).toContain('2026-05-20 M4/M5 Readiness Evidence');
    expect(evidence).toContain('2026-05-21 SDK 0.33.31 Handoff Status');
    expect(evidence).toContain('2026-05-21 SDK 0.33.32 Handoff Status');
    expect(evidence).toContain('2026-05-21 SDK 0.33.33 Handoff Status');
    expect(evidence).toContain('2026-05-21 SDK 0.33.34 Handoff Status');
    expect(evidence).toContain('Manual PTY smoke');
    expect(evidence).toContain('AGENT_SMOKE_ONE');
    expect(evidence).toContain('no default wiki fallback or client-side content filtering');
    expect(evidence).toContain('default `/api/knowledge/ask` still answers');
    expect(evidence).toContain('Live delegation dry receipt was skipped');
    expect(risks).toContain('SDK `0.33.34` Agent knowledge validation');
    expect(risks).toContain('Default regular `/api/knowledge/ask` still answers');
    expect(risks).toContain('empty Agent store returns no unrelated default-wiki facts');
