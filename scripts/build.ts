 ...
 *
 * Targets:
 *   linux-x64    bun-linux-x64    → dist/goodvibes-agent-linux-x64
 *   linux-arm64  bun-linux-arm64  → dist/goodvibes-agent-linux-arm64
 *   darwin-x64   bun-darwin-x64   → dist/goodvibes-agent-macos-x64
 *   darwin-arm64 bun-darwin-arm64 → dist/goodvibes-agent-macos-arm64
 */
...
const TARGETS: Record<string, { bunTarget: string; outfile: string; sqliteVecPackage: string }> = {
  'linux-x64': { bunTarget: 'bun-linux-x64', outfile: 'goodvibes-agent-linux-x64', sqliteVecPackage: 'sqlite-vec-linux-x64' },
  'linux-arm64': { bunTarget: 'bun-linux-arm64', outfile: 'goodvibes-agent-linux-arm64', sqliteVecPackage: 'sqlite-vec-linux-arm64' },
  'darwin-x64': { bunTarget: 'bun-darwin-x64', outfile: 'goodvibes-agent-macos-x64', sqliteVecPackage: 'sqlite-vec-darwin-x64' },
  'darwin-arm64': { bunTarget: 'bun-darwin-arm64', outfile: 'goodvibes-agent-macos-arm64', sqliteVecPackage: 'sqlite-vec-darwin-arm64' },
};
...
function buildTarget(
  ...
  const agentResult = spawnSync('bun', [
    'build', 'src/main.ts',
    '--compile',
  ...
  if (agentResult.status !== 0) {
    console.error(`  AGENT FAILED (exit ${agentResult.status})`);
    return false;
  }
  ...
  if (nativeFilename !== undefined) {
    ...
    if (existsSync(srcAddon)) {
      ...
      if (isSamePlatform) {
        ...
        // gate exists to reject). The version is pinned to the resolved
        // sqlite-vec meta-package so every platform addon matches the wrapper.
  ...
  if (size >= 0) {
    console.log(`  AGENT OK — ${formatBytes(size)}`);
  } else {
    console.log('  AGENT OK (size unknown)');
  }
