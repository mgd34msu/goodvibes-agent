...
- connected-host compatibility: checked through public Agent routes; `AGENT_DAEMON_BUILD_FLOOR` (`src/runtime/daemon-build-compatibility.ts`) is `1.28.0` — a breaking change from the daemon/TUI product split, an attached daemon older than that build gets a one-time "update the daemon" notice, and this floor's value and rationale belong in this repository's CHANGELOG and release notes whenever it is raised, not left to infer
