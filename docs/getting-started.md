...
- Memory & Skills: local memory, notes, personas, skills, routines, learned behavior capture, and a read-only learning curator for review/setup/stale/reviewed-note/completed-work memory and behavior candidates.
...
- Memory & Skills -> Learning curator to rank stale, low-confidence, missing-setup, reviewed-note memory/behavior proposals, completed-work memory/behavior proposals, and review-needed local behavior records before they silently guide the assistant.
