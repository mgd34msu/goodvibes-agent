...
| Local model choice | `agent_harness mode:"model_routing" query:"local"`, `mode:"model_routing" includeParameters:true`, `mode:"model_route" modelRouteId:"local-model-cookbook"`; the local cookbook includes endpoint candidates, exact endpoint inspection through `model_route`, model-list smoke criteria, and the `account-local-server-health` workspace action route |
