...
| Need | Tool Or Mode |
| --- | --- |
| Choose the best route for a user task | `route action:"plan" query:"..."`; local background commands route through `execution action:"processes"`; file undo/redo routes through `execution action:"recovery"`; screenshot/browser/desktop-control tasks route through `computer action:"plan"`; lower-level `agent_harness mode:"route_decision"` remains available for compatibility/detail |
