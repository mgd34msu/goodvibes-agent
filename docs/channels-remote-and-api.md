...
VIBE.md, project context files, Agent-local memory, routines, skills, and personas are not automatically broadcast to channels. External delivery is an effect and requires an exact command, an explicit routine promotion, or a user-approved connected-host request. Companion pairing uses `/pair` without printing the raw token; manual token display requires `/pair --show-token --yes`. One-off delivery uses Agent Workspace -> Channels -> Send channel message or `/channels send ... --yes`; recent receipts are visible through Agent Workspace -> Channels -> Delivery receipts or `/channels deliveries`.
...
The model can inspect channel-facing workspace actions with `agent_harness`, list the structured channel readiness map with `agent_harness` mode `channels`, inspect one channel with mode `channel`, inspect recent redacted delivery receipts with mode `channel_deliveries`, and inspect the ordered channel setup guide with mode `channel_setup_guide` using `channelId`, `target`, or `query`. Channel discovery is compact by default, and `workspace_actions` rows include `modelRoute` hints for send/setup actions. Use `includeParameters:true`, single-channel inspection, the receipt ledger, or the setup guide when the model needs delivery target shape, read-only connected-route hints, current setup step, allowlist review route, live-check route, recent confirmed-send outcome ids, or full route metadata. These modes are read-only: they return setup state, delivery posture, risk labels, safe config-key names, default-target key names, and redacted receipt summaries without printing secret values or sending messages. The model can send one explicit configured delivery through `agent_channel_send` only when the user asks for that exact effect. `agent_review_packet_share` is the specialized confirmed route for sending a reviewer packet archive reference through a configured channel target without attaching ZIP bytes. It must not create routes, authorize accounts, infer recipients, or expose new public surfaces from chat.
