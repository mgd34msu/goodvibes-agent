...
- Make setup complete the user's outcome: installed, reachable, authenticated, paired, and ready to act. The current setup plan exposes live probe-fed connected-host diagnostics/status recommendations, token-safe connected-host auth posture, confirmed service repair routes that stay inspect-first until service status proves need, and offline GoodVibes host bootstrap commands before the operator endpoint is reachable; end-to-end install smoke and exact pairing handoff still need depth.
