...
| Closed learning loop | Skills and memory | Autonomous memory, skills, session search, user model | Persistent vector/keyword memory and skills | Partial: local libraries and read-only curator with duplicate-consolidation candidates, visible diffs/rollback routes, reviewed-note, completed-work, completed-research, and saved-session memory/behavior proposals exist; guided batch application still needs depth | Better |
| Autonomous schedules/background work | Cron, wakeups, webhooks, triggers | Built-in cron with delivery | Scheduled tasks and reminders | Partial: confirmed natural-language autonomous schedule creation is available when task, cadence, success criteria, and user request provenance are explicit; read-only ongoing-work intake selects safe routes; autonomy queue includes live research runs, connected-host tasks, approvals, automation runs, schedules, tails, source ids, normalized available/unavailable controls, and exact confirmed control routes; richer host telemetry still needs depth | Better |
...
- Extend the learning curator from duplicate-consolidation planning into guided batch application with explicit update, stale, delete, and rollback confirmation.
- Turn research from the current visible run ledger and browser-readiness posture into a browser-backed live report workflow with artifacts, cancellation, richer visual reports, and explicit knowledge ingest.
