...
| Autonomous schedules/background work | Cron, wakeups, webhooks, triggers | Built-in cron with delivery | Scheduled tasks and reminders | Partial: confirmed natural-language autonomous schedule creation is available when task, cadence, success criteria, and user request provenance are explicit; read-only ongoing-work intake selects safe routes; autonomy queue includes live research runs, connected-host tasks, approvals, automation runs, schedules, tails, source ids, and exact confirmed control routes; richer host telemetry still needs depth | Better |
