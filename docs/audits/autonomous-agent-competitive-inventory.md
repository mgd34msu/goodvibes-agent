...
| First-run and always-on setup | Onboard can install gateway service | Installers and setup wizard configure dependencies and gateway | Docker/native start with admin bootstrap | Partial: route-backed setup plan exists with local model readiness, probe-fed connected-host diagnostics/status recommendations, token-safe connected-host auth posture with exact pairing route ids, inspect-first service install/start/restart routes, offline missing-host bootstrap commands, and setup-state fixtures; end-to-end install smoke still needs depth | Better |
...
| Closed learning loop | Skills and memory | Autonomous memory, skills, session search, user model | Persistent vector/keyword memory and skills | Partial: local libraries and prompt injection now require reviewed/confident memory plus reviewed setup-ready skills, routines, bundles, and personas, while suppressed behavior stays visible for curator review; read-only curator with duplicate-consolidation candidates, visible diffs/rollback routes, an ordered batch review plan, reviewed-note, completed-work, completed-research, and saved-session memory/behavior proposals exists; apply helpers and deeper score-based ordering still need depth | Better |
| Autonomous schedules/background work | Cron, wakeups, webhooks, triggers | Built-in cron with delivery | Scheduled tasks and reminders | Partial: confirmed natural-language autonomous schedule creation is available when task, cadence, success criteria, and user request provenance are explicit; read-only ongoing-work intake selects safe routes; autonomy queue includes live research runs, connected-host tasks, approvals, automation runs, schedules, tails, source ids, normalized available/unavailable controls, and exact confirmed checkpoint/pause/resume/cancel control routes where supported; richer host telemetry still needs depth | Better |
| Computer use, browser, shell | Browser, canvas, nodes, system.run | Terminal backends, browser, code execution, computer use | opencode with web/files/shell/MCP | Partial: local-first execution posture routes read/edit/exec/web/delegation, supervision routes, bounded local execution history records, confirmed file recovery, and strict browser/desktop ready-attention-setup workflow cards; first-class adapters still pending | Better |
| Multi-agent and remote execution | Multi-agent routing and session tools | Subagents, kanban, worktrees | Agent runs whole tasks | Partial: GoodVibes has shared-session, remote runner, artifact, task, and worktree foundations; Agent now exposes local-first, TUI handoff, delegated-review, remote-inspection, and hidden-fanout-blocked decision cards plus structured confirmed handoff briefs, but does not yet run cancellable multi-runner work plans from Agent | Better |
...
2. The largest gap is not raw capability count. It is UX integration. The current Agent now exposes the same assistant-first cockpit in summary and the visible Home view; remaining gaps are depth in setup completion, live connectors, browser-backed research, and measured local model serving.
...
- Make setup complete the user's outcome: installed, reachable, authenticated, paired, and ready to act. The current setup plan exposes live probe-fed connected-host diagnostics/status recommendations, token-safe connected-host auth posture with exact pairing route ids and host-owned provisioning boundaries, confirmed service repair routes that stay inspect-first until service status proves need, and offline GoodVibes host bootstrap commands before the operator endpoint is reachable; end-to-end install smoke and host-side token provisioning contract still need depth.
...
- Extend live autonomy queue records with deeper host output streams, diagnostics, and pause/resume lifecycle controls when the connected host exposes them.
...
- Extend the learning curator from duplicate-consolidation batch review planning into safe apply helpers with explicit update, stale, delete, and rollback confirmation.
