...
| Documents and model comparison | Canvas/web primitives | TUI/dashboard/session tools | Documents and blind Compare | Partial: Document Ops maps uploads/export/artifacts/sources and has a confirmed blind runner with durable JSON artifacts and saved review boards; no dedicated editor, saved preference handoff, or unified artifact browser | Parity |
...
4. Email, calendar, dedicated document editing, saved model-comparison preferences and route handoff, deep research reports, local model serving recommendations, and browser/computer-use setup are the clearest product gaps.
...
- Promote Document Ops from readiness map into a real document editor, versioned artifact browser, saved comparison preferences, and route-update handoff.
