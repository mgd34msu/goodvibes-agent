...
| Autonomous schedules/background work | Cron, wakeups, webhooks, triggers | Built-in cron with delivery | Scheduled tasks and reminders | Partial: read-only autonomy queue includes live research runs, connected-host tasks, approvals, automation runs, schedules, tails, source ids, and exact confirmed control routes; natural-language recurring task creation and richer host telemetry still need depth | Better |
