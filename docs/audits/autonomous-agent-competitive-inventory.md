...
| First-run and always-on setup | Onboard can install gateway service | Installers and setup wizard configure dependencies and gateway | Docker/native start with admin bootstrap | Partial: route-backed setup plan exists with local model readiness, probe-fed connected-host diagnostics/status recommendations, inspect-first service install/start/restart routes, and an offline missing-host bootstrap plan with GoodVibes host install/trust/verify/service/reconnect commands; end-to-end install smoke still needs depth | Better |
...
- Make setup complete the user's outcome: installed, reachable, authenticated, paired, and ready to act. The current setup plan exposes live probe-fed connected-host diagnostics/status recommendations, confirmed service repair routes that stay inspect-first until service status proves need, and offline GoodVibes host bootstrap commands before the operator endpoint is reachable; end-to-end install smoke still needs depth.
