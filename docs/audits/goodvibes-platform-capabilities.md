...
- `agent_harness mode:"autonomy_intake"` now conservatively maps ongoing-work requests to the safest visible route and missing fields before any confirmed effect.
...
- Confirmed natural-language recurring task creation beyond the current conservative route intake selector.
...
- First-class browser/desktop-control adapters and recovery beyond the current route-backed setup posture, local-first execution posture for read/edit/exec, web fetch, supervision routes, confirmed file edit recovery, and delegation routing.
