...
- Durable setup smoke evidence artifacts and host-side token provisioning contract after the missing-host/missing-token bootstrap path; setup already exposes live service probe evidence, token-safe connected-host auth posture with exact pairing route ids, offline GoodVibes host install/trust/verify/service/reconnect commands, confirmed token-safe setup smoke execution, recommended diagnostics/status cards, setup-state fixtures, and confirmed service install/start/restart routes that stay inspect-first until service status proves need.
