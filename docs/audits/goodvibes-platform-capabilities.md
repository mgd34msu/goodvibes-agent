...
- End-to-end install smoke and exact pairing handoff after the missing-host/missing-token bootstrap path; setup already exposes live service probe evidence, token-safe connected-host auth posture, offline GoodVibes host install/trust/verify/service/reconnect commands, recommended diagnostics/status cards, setup-state fixtures, and confirmed service install/start/restart routes that stay inspect-first until service status proves need.
