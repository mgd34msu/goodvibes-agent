...
- `agent_harness mode:"autonomy_intake"` maps ongoing-work requests to the safest visible route and missing fields before any confirmed effect; `agent_autonomy_schedule` creates one visible connected autonomous schedule when task, cadence, success criteria, and request provenance are explicit.
