...
- Documents & Compare now exposes a confirmed Save packet preset form backed by `agent_review_packet_presets`; presets are local JSON artifacts that remember document/export/comparison/judgment/route-decision/handoff/archive/related ids, appear in the packet timeline with attention state when stale, only fill defaults when live packet evidence is missing, and list/show freshness checks flag missing or superseded artifact ids with safer reuse routes when newer matching evidence is available.
