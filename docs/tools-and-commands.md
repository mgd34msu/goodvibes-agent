...
| `provider_accounts`, `model_routing`, `execution_posture`, `execution_history`, `file_recovery` | Provider auth, provider/model route posture, readiness scores, hardware-scored local model cookbook with setup plans, benchmark prompts/history, local-vs-delegated execution routing, redacted execution records, and file edit recovery. |
...
| `mcp_servers`, `setup_posture`, `pairing_posture`, `delegation_posture` | MCP, first-run setup plan with probe-fed connected-host repair/auth cards and local model readiness, pairing, and build-delegation posture. |
