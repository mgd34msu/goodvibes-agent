...
| `personal_ops`, `personal_ops_intake`, `personal_ops_lane` | Inbox/calendar connector readiness, request intake that chooses the safest lane/route/fields/confirmation boundary, classified MCP read/write tool hints, schema-derived operation records, triage/draft/agenda/conflict workflow cards, matching MCP setup routes, and live Agent-owned note, routine, schedule-receipt, and delivery records. |
