...
Use `device action:"browser"` for the connected browser cockpit/PWA readiness summary and `device action:"open_browser" confirm:true explicitUserRequest:"..."` for the visible browser handoff. `workspace action:"surfaces|surface|open"` is the normal visible UI route; lower-level `ui_surfaces`/`ui_surface` remain modal, picker, overlay, and workspace catalog detail routes. The connected browser cockpit/PWA is `surfaceId:"connected-browser-cockpit"`; it resolves the configured connected-host web URL, opens only through confirmed `device action:"open_browser"` or `workspace action:"open"`, returns service/web setup routes when disabled, and reports workspace category coverage, mobile/PWA controls, Agent onboarding marker status, and the currently unpublished browser/PWA first-run receipt contract.
