...
| `provider_accounts`, `model_routing`, `execution_posture`, `execution_history`, `file_recovery` | Provider auth, provider/model route posture, readiness scores, hardware-scored local model cookbook with setup plans and benchmark prompts, local-vs-delegated execution routing, redacted execution records, and file edit recovery. |
