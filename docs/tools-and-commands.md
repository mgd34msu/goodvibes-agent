...
| `support_bundles`, `media_posture`, `sessions` | Redacted support bundle posture, voice/media posture, and saved session/bookmark posture; route planning prefers support bundles for diagnostics packets and sessions for saved transcript/search/resume/export wording before confirmed lifecycle effects. |
...
| `release_evidence`, `release_readiness` | Operator/audit release artifacts and release-quality inventory; route planning sends release evidence, readiness, gate, verification ledger, and audit wording to these read-only surfaces. |
