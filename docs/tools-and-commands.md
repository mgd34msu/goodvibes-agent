...
- Read-only learning review uses `agent_harness mode:"learning_curator"` and `mode:"learning_candidate"`; reviewed-note behavior proposals reuse selected-note promotion routes, and writes stay on `agent_local_registry` or visible workspace actions.
