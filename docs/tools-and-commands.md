...
| `route action:"plan"` plus lower-level `route_decision` | User-task route planning across Agent setup, Personal Ops, research, autonomy, execution, delegation, computer/browser, workspace, host, device, channel, security, Local Context, external memory-provider setup, and Knowledge surfaces. |
