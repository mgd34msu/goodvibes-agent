...
The model can inspect service posture with `agent_harness` mode `service_posture`. That report exposes endpoint binding, network-facing posture, issues, and redacted-log diagnostics used by status, doctor, and support bundles. Use `includeParameters:true` when reachability probes and redacted log tail are needed. To inspect one endpoint, use `service_endpoint` with `endpointId`, `target`, or `query`; valid endpoint ids are `controlPlane`, `httpListener`, and `web`. The connected browser cockpit/PWA is the visible UI surface `connected-browser-cockpit`; confirmed `open_ui_surface` resolves `web.publicBaseUrl` or the web endpoint binding and opens the configured URL, while disabled web posture returns service/setup routes instead. `ui_surface surfaceId:"connected-browser-cockpit"` also reports Agent workspace category coverage, mobile/PWA control routes, the Agent onboarding marker, and the unpublished browser/PWA first-run receipt contract. Service lifecycle and listener changes must use setup or confirmed daemon operator methods.
