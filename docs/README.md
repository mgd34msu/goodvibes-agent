...
- `agent_documents`, `agent_artifacts`, `agent_research_runs`, `agent_research_sources`, and `agent_research_report` for versioned Agent document drafts, review comments, AI suggestion review, saved artifact attachment/insertion, markdown artifact export, saved artifact browsing/export/package/archive, project-local visible research run state with log tails, `agent_harness mode:"research_workflow"` route planning, research source review, reviewed-source bundles, sourced research report artifacts with citation coverage repair hints, and reviewed artifact-to-Knowledge promotion.
