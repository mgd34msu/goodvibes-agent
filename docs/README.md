...
- `agent_documents`, `agent_artifacts`, `agent_research_runs`, `agent_research_sources`, and `agent_research_report` for versioned Agent document drafts, review comments, AI suggestion review, saved artifact attachment/insertion, reviewer-ready markdown artifact export with comment and suggestion summaries, saved artifact browsing/export/package/archive, project-local visible research run state with log tails, `agent_harness mode:"research_workflow"` route planning, research source review, reviewed-source bundles, sourced research report artifacts with citation coverage repair hints, and reviewed artifact-to-Knowledge promotion.
- `agent_channel_send`, `agent_notify`, `agent_autonomy_schedule`, `agent_reminder_schedule`, `agent_schedule_edit`, `agent_media_generate`, and `agent_model_compare` for explicit confirmed effects, including connected autonomous schedules and blind comparison from saved text artifacts with side-by-side reviewer views, split-pane reviewer handoff diffs with section jumps and recent-handoff choices, chronological review packet timeline state, review packet wizard progress/routes, route-decision receipt artifacts for apply/leave-unchanged evidence, packet-default form prefill for export/handoff/archive/apply workflows, inline reviewer-readiness badges before export/archive/apply, task/document/benchmark-filtered saved judgment analytics/synthesis, reviewer handoff artifacts, and one-click handoff ZIP archives.
