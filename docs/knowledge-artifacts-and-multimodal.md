...
The model can use first-class tools for the common Knowledge, media, and comparison workflows: `agent_knowledge`, `agent_knowledge_ingest`, `agent_media_generate`, and `agent_model_compare`. Blind model comparison saves a local JSON artifact with the full prompt, blinded outputs, and reveal map while keeping model identities hidden in the transcript until reveal; saved artifacts can be reopened as blind review boards from Documents & Compare, then converted into confirmed judgment artifacts without changing model routing. `agent_harness` adds workspace-action lookup/execution, compact `modelRoute` hints in `workspace_actions`, `document_ops`/`document_ops_lane` readiness, `media_posture`/`media_provider` readiness, `sessions`/`session` bookmark and artifact posture, and confirmed slash-command mirrors when a visible workspace route maps to a concrete command.
