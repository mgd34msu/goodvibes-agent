...
- `agent_harness` modes `model_routing`, `model_route`, and confirmed `run_local_model_smoke` when the model needs provider/model route posture, selectable model metadata, pinned model status, reasoning support, context-window posture, readiness scores, safe setting keys, and the hardware-scored local model cookbook. `model_routing` is compact by default; use `includeParameters:true` or `model_route` for full capabilities, readiness dimensions, local hardware profile, fit scores, setup/download guidance, local endpoint candidates with exact endpoint inspection, model-list smoke commands, success criteria, failure triage, confirmed local model-list smoke results, confirmed benchmark action routes, saved benchmark evidence, and route-change hints. Workspace model actions also expose compact `modelRoute` hints for settings, pin/unpin, reasoning effort, route readiness inspection, local model recommendations, local server checks, benchmark execution, saved benchmark evidence review, and visible picker flows.
